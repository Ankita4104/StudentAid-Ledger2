...
import { Student, FinancialRequest, Donation } from '../types';
...
const Dashboard: React.FC<DashboardProps> = ({ user, requests, donations }) => {
  ...
  const totalRaised = requests.reduce((sum, r) => sum + r.raisedAmount, 0);
  ...
  return (
    <div className="space-y-16 animate-reveal max-w-5xl mx-auto px-4 pb-20">
      <div className="flex flex-col gap-3 items-center text-center sm:items-start sm:text-left">
        ...
        <p className="text-emerald-400 font-bold uppercase tracking-[0.3em] text-[10px]">Active Verified Member • {user?.name}</p>
      </div>
                ...
                requests.map(req => (
                  <tr key={req.id} className="hover:bg-emerald-50/30 transition-colors group">
                    ...
                    <td className="px-12 py-9 text-right font-black text-emerald-900">
                      <span className="text-2xl">₹{req.raisedAmount.toLocaleString()}</span>
                      <span className="text-emerald-200 text-sm ml-2">/ ₹{req.requestedAmount.toLocaleString()}</span>
                    </td>
                  </tr>
                ))
              ) : (
      ...
      <div className="space-y-10">
        ...
        <div className="grid gap-6">
          {donations.length > 0 ? (
            donations.map(don => (
              <div key={don.id} className="pretty-card p-10 rounded-[2.5rem] flex justify-between items-center group">
                <div className="flex items-center gap-8">
                  ...
                  <div>
                    <p className="font-black text-emerald-950 text-xl tracking-tight">Gift to Request #{don.requestId.slice(0, 8)}</p>
